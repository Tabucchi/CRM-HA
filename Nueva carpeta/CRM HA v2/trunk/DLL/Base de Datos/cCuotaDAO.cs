...
namespace DLL.Base_de_Datos
{
    public class cCuotaDAO
    {
        ...
        public cCuota GetFirstActiva(string idCC, string _idFormaPago)
        {
            ...
            DateTime max = new DateTime(date.Month == 12 ? date.Year + 1 : date.Year, date.Month == 12 ? 1 : date.Month + 1, 1).AddDays(-1);

            string query = "SELECT Top(1) id FROM tCuota WHERE idCuentaCorriente=" + idCC + " AND idFormaPagoOV = '" + _idFormaPago + "'";
            query += " AND estado='" + (Int16)estadoCuenta_Cuota.Activa + "' AND fechaVencimiento1 BETWEEN @fechaDesde AND @fechaHasta";
